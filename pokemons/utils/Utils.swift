...
class Utils {
    ...
    private static var caught: [Pokemon]?
    ...
    private static func readCaught() {
        if let storedObject: Data = UserDefaults.standard.data(forKey: "caught") {
            do {
                ...
                print("Error fetching data from user defaults: \(error.localizedDescription)")
                caught = []
            }
        } else {
            print("No pokemon in user defaults")
            caught = []
        }
    }
    
    private static func writeCaught() -> Void {
        do {
    ...
    static func toggleCaught(_ pokemon:Pokemon) {
        if caught == nil {
            readCaught()
        }
